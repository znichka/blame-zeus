...
│       └── resources/
│           ├── application.yml
│           └── db/migration/   (Flyway V1–V17 incl. V8_1–V8_4, V9_1–V9_2, V14_1 + afterMigrate callback; V10–V12 generated by ingestion/seedgen)
...
**SQL tables (Flyway V1–V17, plus V8_1–V8_3 provenance/normalization additions per DEV-021/022/023, V8_4 embedding upgrade per DEV-028/ADR-013, V9_1 `entities.subtype` + V9_2 `birth`→`parentage` alias per DEV-040/DEV-042, V14_1 epithet/spelling aliases per DEV-100, V15/V16 schema-comment clarifications per DEV-047/DEV-054, and V17 `relation_aliases` per ADR-019):**
```
...
variant_claims(id, subject_entity_id→entities, claim_type, claim_value, source_id TEXT→sources, trust_tier SMALLINT, passage_ref)
  ...
  --   subject+claim_type and ConflictSynthesizer formats them. So a single-source contradiction is not
  --   "detected" as a conflict yet still surfaces at query time. Floor case: Apollodorus ALONE gives Io
  --   three different fathers (Inachus / Piren / Iasus), which the >=2-distinct-sources detector cannot see.
  --   (The Io/parentage group as a whole is no longer single-source -- it also holds Ovid rows; the
  --   Apollodorus-internal disagreement is the part the detector misses.)
