...
- [ ] **A1** `ingestion/requirements.txt` — `openai>=1.0`, `psycopg2-binary`, `pgvector`,
      `tenacity>=8.2`, `python-dotenv`, plus `pytest>=8.0` for the test track
- [ ] **A2** Decide `pyproject.toml` vs. `requirements.txt`-only (plan allows either) — if
      skipping `pyproject.toml`, note the decision inline in `requirements.txt`'s header comment
- [ ] **A3** Create venv (`python3.12 -m venv .venv`) and `pip install -r requirements.txt`
- [ ] **A4** `ingestion/config.py` — reads env vars via `python-dotenv`: `OPENAI_API_KEY`,
...
- [ ] **A5** `ingestion/tests/__init__.py` (empty, makes the dir a package for pytest discovery)
- [ ] **A6** Confirm `ingestion/.venv/` (or wherever the venv lands) is `.gitignore`d
...
- [ ] **B3** Manual QA pass: confirm no leftover HTML tags/entities, markers are intact and in
