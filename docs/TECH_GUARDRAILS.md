...
| **No live LLM calls in tests** | All `@AiService` interfaces must be mocked. Use `mockk<T>()` for pure unit tests and `@MockkBean` (`com.ninja-squad:springmockk`) for Spring-context tests. **Scope (ADR-018):** this rule governs the automated Gradle/CI test suite (`:core-api:test`) only. It does **not** apply to developer-invoked **offline operator tools** run against a live server — the evaluation harness (`evaluation/runner/`) and the `ingestion` job — which are outside the build/CI and legitimately call live models. Such tools must never live in a test sourceset. |
