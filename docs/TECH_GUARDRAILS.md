...
| **Embedding dimension = 1536** | Tied to OpenAI `text-embedding-3-small`. Ingestion writes these embeddings; retrieval must use the same model and dimension. The embedding model in `LangChain4jConfig.kt` is intentionally OpenAI-specific and not swappable without re-ingesting the full corpus. The chat model, by contrast, is provider-configurable. |
| **`pg_trgm` extension required** | `V1__enable_pgvector.sql` enables both `vector` and `pg_trgm`. `V3__create_entities.sql` adds a GIN trigram index on `entities.name`. Used by `ConflictQueryHandler` step 3 for fuzzy entity name matching (spelling variants, typos). |
...
| **HNSW index** | Use `USING hnsw (embedding vector_cosine_ops)` — not IVFFlat. IVFFlat requires a minimum number of rows to build; HNSW works at any size. |
...
| **One handler per route** | `SqlQueryHandler`, `RagQueryHandler`, `ConflictQueryHandler`, `MixedQueryHandler` — each in its own file. `QueryService` is the only place that knows about all four. |
...
LLM_API_KEY=sk-...             # used by core-api chat model bean (app.llm.chat-api-key); set equal to OPENAI_API_KEY for Phase 1
LLM_CHAT_MODEL=gpt-4o-mini     # example value only — no default is set in application.yml; update LangChain4jConfig.kt beans when using a different chat provider
