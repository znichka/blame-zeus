...
|--------------|-------------|
| **Date**     | 2026-07-31 |
| **Status**   | **Accepted — implemented and landed 2026-08-01 by Stage P6** (`docs/TODO-phase2-stage-p6.md`, DEV-140/142/143/144/145/146/147). What landed vs. what this ADR proposed: **rule 1 (ledger)** as written — 34,654 rows, every resolution carrying its corpus location (DEV-141), with one addition the ADR did not specify: a memo hit re-reports the layer that *established* the identity, without which G2's denominator undercounts 7.6x and G6's signal goes blind after a name's first occurrence. **Rule 2 (registry)** as written, including 'first, ahead of the exact-match memo' — but G3.2a's prescribed `_seen` re-key was achieved by the registry holding **no state at all**, same invariant, less machinery (DEV-144); 63 entries, every one hand-adjudicated with a stated reason. **Rule 3 (`Name (descriptor)` splitting)** as written. **Rule 4 (merge gate)** as written, and **measured**: the specified HIGH rule scores 65% recall at **19% precision** over 7 adjudicated passages, so it ships unchanged as a *label* while an asymmetry key does the *ordering* (DEV-146). **The fuzzy step was demoted**, not kept: measured at exactly 70.0% false positives across a stratified sample of 50, meeting the pre-registered threshold (DEV-143). The ADR's own hypothesis that 'the fuzzy step's entire live band may be false positives' is **not** supported — 30% of the sample were genuine merges, all 15 restored via `known_aliases.json`. |
